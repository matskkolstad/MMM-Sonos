...
Module.register('MMM-Sonos', {
  ...
  getDom() {
    ...
    if (isFullscreenMode) {
      ...
      // Idle speakers are kept as cards in touch control mode (controlShowIdleZones).
      groupsToRender = this.groups
        .filter((group) => this._isGroupVisible(group) || (!isMiniMode && this._isIdleControlZone(group)))
  ...
  _getHiddenIdleZones() {
    ...
    // Speakers this instance may show but that are not playing (listed behind "+").
    return (this.groups || []).filter((group) => !this._isHidden(group) && !this._isGroupVisible(group));
  },
  ...
  // Touch control mode shows speakers that are not playing as idle cards (row/grid),
  // so they can be started from the mirror.
  _isIdleControlZone(group) {
    if (!group || !this.config.enableControls || !this.config.controlShowIdleZones || this._isHidden(group)) {
      return false;
    }
    return !this._isGroupVisible(group);
  },
